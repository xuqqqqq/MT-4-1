...
def solve(input_text: str) -> list:
    ...
    def high_courier_anchor_triple(result, assigned_couriers, start_time, time_limit=9.5):
        n = len(result)
        anchor_indices = []
        other_indices = []
        for idx in range(n):
            group, couriers = result[idx]
            if len(group.split(",")) == 1 and len(couriers) >= 4:
                anchor_indices.append(idx)
            elif len(group.split(",")) == 1 and 1 <= len(couriers) <= 4:
                other_indices.append(idx)
        if not anchor_indices or len(other_indices) < 2:
            return
        other_info = []
        for idx in other_indices:
            ...
            selected = [(s, cid, w) for s, cid, w in cands if cid in couriers]
            pen = expected_penalty(selected)
            other_info.append((pen, idx, group, couriers))
        other_info.sort(key=lambda x: x[0], reverse=True)
        top40 = other_info[:40]
        if len(top40) < 2:
            return
        for anchor_idx in anchor_indices:
            if time.time() - start_time > time_limit:
                return
            group_a = result[anchor_idx][0]
            couriers_a = result[anchor_idx][1]
            for i in range(len(top40)):
                for j in range(i + 1, len(top40)):
                    ...
                    idx_b = top40[i][1]
                    idx_c = top40[j][1]
                    if idx_b == anchor_idx or idx_c == anchor_idx:
                        continue
                    group_b = result[idx_b][0]
                    group_c = result[idx_c][0]
                    couriers_b = result[idx_b][1]
                    ...
                    for sub_a, pen_a in subsets_a:
                        ...
                        for sub_b, pen_b in subsets_b:
                            ...
                            for sub_c, pen_c in subsets_c:
                                ...
                                new_pen = pen_a + pen_b + pen_c
                                if new_pen < old_pen - 1e-7:
                                    result[anchor_idx] = (group_a, sub_a)
                                    result[idx_b] = (group_b, sub_b)
                                    result[idx_c] = (group_c, sub_c)
                                    assigned_couriers.clear()
                                    for _, couriers in result:
