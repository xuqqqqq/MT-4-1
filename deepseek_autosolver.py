...
def build_generation_prompt(
    ...
    prompt_parts = [
        ...
        "Hard line budget: the complete solver must be under 900 physical lines.",
        "Any solution that is truncated, unfinished, or over 900 lines is invalid.",
...
def build_strategy_reflection_prompt(
    ...
    prompt_parts = [
        ...
        "Current DeepSeek target on large_seed301 is <= 650.0; tying the 673.370431 incumbent is not progress.",
        "",
        "Important empirical memory from this DeepSeek branch:",
        "- Official greedy baseline on large_seed301: about 2097.66.",
        "- Earlier non-hardcoded DeepSeek solver on large_seed301 reached about 678.40; current retained best is 662.6273661397067.",
        ...
        "- Latest black-box feedback: a too-compressed one-courier relocation rewrite lost the parent and emitted invalid pair keys; relocation is promising only if added as a true parent-preserving pass.",
        "- Latest black-box feedback: adding a true one-courier relocation pass to the light 669.70 parent improved large_seed301 to 667.1756 in about 1.24s, better and faster than the 3-task enumeration.",
        "- Latest black-box feedback: a post-relocation refinement rewrite regressed to 1129.36 with invalid pair keys; do not rewrite the group construction or create comma-joined keys.",
        "- Latest black-box feedback: narrowed 3-group local pool redistribution after relocation improved to 666.2408 in about 6.39s; broad 3-pool timed out, so keep pool filters tight.",
        "- Latest black-box feedback: a second 3-pool pass improved to 665.4769 but is runtime-risky at about 9.7-9.9s; an attempted speed version regressed to 666.41.",
        "- Latest black-box feedback: adding a post-pool first relocation improved to 665.4048 and reduced runtime to about 6.75s, making it the current best retained parent.",
        "- Latest black-box feedback: after 665.4048, diagnostics found another 3-group pool improvement around 1.06 involving one group with only 1 current courier; 3-pool filters that require every group to have 2-3 couriers miss this pattern.",
        "- Latest black-box feedback: adding a final 3-pool pass that allows 1..3 current couriers per group improved to 664.6222 in about 6.28s.",
        "- Latest black-box feedback: appending yet another singleton-aware 3-pool pass produced 744 lines and timed out; future variants should consolidate existing passes instead of appending more scans.",
        "- Latest black-box feedback: a restricted singleton-priority 3-pool pass ran fast but regressed to 664.75, so future final passes must full-score-check against the parent before returning.",
        "- Latest black-box feedback: broad 3-group courier chain relocation timed out; any chain version must use strict candidate filtering and parent fallback.",
        "- Latest black-box feedback: the safest useful improvement was not a new neighborhood but repeating the existing final_three_group_pool once; it improved 664.6222 -> 663.5624 in about 8.1s.",
        "- Latest black-box feedback: adding one generic top-20 high-penalty first-improvement triple pass after the two final pool calls improved further to 663.4691 in about 8.5s; a second call was neutral, and a best-improvement top-20 pass timed out.",
        "- Latest diagnostics after 663.4691 show another possible top-40 triple gain around 0.186, but the full top-40 scan costs about 2.4s after solving and is too close to the 10s limit unless a much sharper priority order is found.",
        "- Latest black-box feedback: a generic top-12 high-penalty anchor-pair plus arbitrary third-group first-improvement pass captured that 0.186 gain and improved to 663.2831 in about 8.2-8.8s.",
        "- Latest black-box feedback: a top-20 first-improvement pass that allows one-task groups with 1..4 current couriers and pool<=7 improved further to 663.2124 in about 8.5-8.9s; it captured an earlier 4-courier move, not the full best 0.210 diagnostic gain.",
        "- Latest black-box feedback: a high-current-courier anchor pass for single-task groups with len(couriers)>=4 captured the larger 4-courier redistribution and improved to 662.6823 in about 8.4-8.7s.",
        "- Latest black-box feedback: a compact late one-courier transfer pass captured the remaining 0.05495 move and improved to 662.6274; parallel local runs were about 9.34s, but a sequential run was about 8.57s.",
        "- Latest black-box feedback: direct pair-bundle replacement was exhausted locally (all 780 direct pair merges and 29,628 pair+one-task repairs had zero positive moves), so do not retry simple merge-single-into-pair.",
        "- Latest black-box feedback: a pair-bundle perturb-and-repair candidate regressed to 671.80 and ran at 9.98s; pair bundle changes need a stronger global construction, not a late perturbation graft.",
        "- Latest ablation feedback: removing any of the current late passes loses score (typical ablations landed around 662.68, 663.07, or 663.47), so there is no obvious redundant late pass to delete for free time.",
